@extends('layouts.app')
@section('title', 'Tambah Hutang - FinanceHub')
@section('header-title', 'Tambah Hutang Usaha')
...
<div class="view-section active" style="display: block; padding: 20px; background: white; border-radius: 12px; margin-top: 20px;">
    ...
    <form action="{{ route('hutangs.store') }}" method="POST" style="max-width: 800px;">
        @csrf
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div>
                ...
                <select name="jenis_hutang" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
                    ...
                    @foreach($jenis_hutang as $jh)
                        <option value="{{ $jh }}" {{ old('jenis_hutang') == $jh ? 'selected' : '' }}>{{ $jh }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="kreditur" value="{{ old('kreditur') }}" required placeholder="Contoh: Bank BCA / Tn. Budi" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
            </div>
