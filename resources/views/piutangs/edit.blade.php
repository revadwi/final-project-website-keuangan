@extends('layouts.app')
@section('title', 'Edit Piutang - FinanceHub')
@section('header-title', 'Edit Piutang Usaha')
...
<div class="view-section active" style="display: block; padding: 20px; background: white; border-radius: 12px; margin-top: 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2>Form Edit Piutang: {{ $piutang->nomor_urut }}</h2>
    ...
    <form action="{{ route('piutangs.update', $piutang->id) }}" method="POST" style="max-width: 800px;">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div>
                ...
                <input type="date" name="tanggal_dibuat" value="{{ old('tanggal_dibuat', $piutang->tanggal_dibuat) }}" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
            </div>

            <div>
                <label style="display: block; margin-bottom: 8px; color: #475569; font-weight: 500;">Nomor Urut <span style="color:red">*</span></label>
                <input type="text" name="nomor_urut" value="{{ old('nomor_urut', $piutang->nomor_urut) }}" required placeholder="Contoh: 01/JN/HU/XII/2025" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
            </div>
            ...
            <div>
                ...
                <select name="jenis_piutang" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
                    <option value="" disabled>-- Pilih Akun Piutang --</option>
                    @foreach($jenis_piutang as $jh)
                        <option value="{{ $jh }}" {{ old('jenis_piutang', $piutang->jenis_piutang) == $jh ? 'selected' : '' }}>{{ $jh }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="project_id" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
                    ...
                    @foreach($projects as $p)
                        <option value="{{ $p->id }}" {{ old('project_id', $piutang->project_id) == $p->id ? 'selected' : '' }}>{{ $p->nama_project }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div style="grid-column: 1 / -1;">
                ...
                <textarea name="keterangan" rows="3" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none; resize: vertical;" placeholder="Catatan tambahan (Opsional)">{{ old('keterangan', $piutang->keterangan) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="date" name="jatuh_tempo" value="{{ old('jatuh_tempo', $piutang->jatuh_tempo) }}" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;">
            </div>
            ...
            <div>
                ...
                <div style="position: relative;">
                    ...
                    <input type="number" name="nominal_awal" id="nominal_awal" value="{{ old('nominal_awal', $piutang->nominal_awal) }}" required style="width: 100%; padding: 10px 10px 10px 40px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;" oninput="calculateTotal()">
                </div>
            </div>
            ...
            <div>
                ...
                <div style="position: relative;">
                    ...
                    <input type="number" step="0.01" name="bunga_persen" id="bunga_persen" value="{{ old('bunga_persen', $piutang->bunga_persen) }}" required style="width: 100%; padding: 10px 40px 10px 10px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none;" oninput="calculateTotal()">
                </div>
            </div>
